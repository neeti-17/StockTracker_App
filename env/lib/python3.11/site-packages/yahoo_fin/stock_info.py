...
def tickers_nifty50(include_company_data = False, headers = {'User-agent': 'Mozilla/5.0'}):
    ...
    table = pd.read_html(StringIO(requests.get(site, headers=headers).text))[0]
...
def get_quote_table(ticker, dict_result=True, headers={'User-agent': 'Mozilla/5.0'}):
    ...
    # Request and parse the HTML tables
    tables = pd.read_html(requests.get(site, headers=headers).text)
    
    # Check to ensure at least two tables are available before accessing them
    if len(tables) < 2:
        print(f"Warning: Less than 2 tables found for ticker {ticker}. Returning None.")
        return None  # or you could return an empty DataFrame or handle differently

    data = pd.concat([tables[0], tables[1]], ignore_index=True)
    data.columns = ["attribute", "value"]

    if dict_result:
        return dict(zip(data["attribute"], data["value"]))
    else:
        return data
